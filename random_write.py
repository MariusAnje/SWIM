...
if __name__ == "__main__":
    ...
    parser.add_argument('--rand_run', action='store', type=int, default=100,
            help='# random runs')
    ...
    for _ in range(args.rand_run):
        ...
        mean = np.mean(fine_mask_acc_list)
        std = np.std(fine_mask_acc_list)
        mean_list.append(mean)
        std_list.append(std)
        print(f"Avg: {np.mean(fine_mask_acc_list):.4f}, std: {np.std(fine_mask_acc_list):.4f}")
    print(f"Finetune noise average acc: {np.mean(mean_list):.4f}, std: {np.std(std_list):.4f}")